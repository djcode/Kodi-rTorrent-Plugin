...
def main(digest, file_hash):
    ...
    if g.__islocal__==1:
        url = g.rtc.f.frozen_path(file_hash)
    else:
        f_name = g.rtc.f.path(file_hash)
        dld_name = g.rtc.d.name(digest)
        dld_is_multi_file = int(g.rtc.d.is_multi_file(digest))
        dld_complete = int(g.rtc.d.complete(digest))
    ...
    f_completed_chunks = int(g.rtc.f.completed_chunks(file_hash))
    f_size_chunks = int(g.rtc.f.size_chunks(file_hash))
