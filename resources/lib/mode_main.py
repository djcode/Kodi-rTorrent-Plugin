...
def main():
    ...
    for dld in dlds:
        ...
        cm_startstop_action = 'download_stop' if dld['is_active']==1 else 'download_start'
        cm_startstop_label = g.__lang__(30101) if dld['is_active']==1 else g.__lang__(30100)

        li_name = '{} ({:.1f}%)'.format(dld['name'], dld_percent_complete) if dld_percent_complete<100 else dld['name']

        context_menu = [
            (cm_startstop_label, f.run_string('action', {'method': cm_startstop_action, 'download_hash': dld['hash']})),
            (g.__lang__(30102), f.run_string('action', {'method': 'download_erase', 'download_hash': dld['hash']})),
            (g.__lang__(30120), f.run_string('action', {'method': 'download_priority', 'download_hash': dld['hash'], 'priority': 3})),
            (g.__lang__(30121), f.run_string('action', {'method': 'download_priority', 'download_hash': dld['hash'], 'priority': 2})),
            (g.__lang__(30122), f.run_string('action', {'method': 'download_priority', 'download_hash': dld['hash'], 'priority': 1})),
            (g.__lang__(30123), f.run_string('action', {'method': 'download_priority', 'download_hash': dld['hash'], 'priority': 0}))
        ]
        ...
        if dld['size_files']>1:
            if not xbmcplugin.addDirectoryItem(int(sys.argv[1]),
                f.run_string('files', {'digest': dld['hash'], 'numfiles': dld['size_files']}, False),
                list_item,isFolder=True,totalItems=dlds_len): break
        else:
            if not xbmcplugin.addDirectoryItem(int(sys.argv[1]),
                f.run_string('play',{'digest': dld['hash'], 'file_hash': f.file_hash(dld['hash'], 0)}, False),
