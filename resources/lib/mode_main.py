...
def main():
    # addonMenu();
    dlds = []
    dlds = g.rtc.d.multicall('main', "d.get_name=", "d.get_hash=", "d.get_completed_chunks=", "d.get_size_chunks=",
                             "d.get_size_files=", "d.get_directory=", "d.is_active=", "d.get_complete=",
                             "d.get_priority=", "d.is_multi_file=", "d.get_size_bytes=")
    dlds_len = len(dlds)
    ...
    for dld in dlds:
        ...
        if dld_is_active == 1:
            cm_action = g.__lang__(30101), "xbmc.runPlugin(%s?mode=action&method=d.stop&arg1=%s)" % (
                sys.argv[0], dld_hash)
        else:
            cm_action = g.__lang__(30100), "xbmc.runPlugin(%s?mode=action&method=d.start&arg1=%s)" % (
                sys.argv[0], dld_hash)
        ...
        if dld_size_files > 1:
            if not xbmcplugin.addDirectoryItem(int(sys.argv[1]),
                                               ...
                                               li, isFolder=True, totalItems=dlds_len):
                break
        else:
            if not xbmcplugin.addDirectoryItem(int(sys.argv[1]),
                                               sys.argv[0] + "?mode=play&arg1=0&hash=" + dld_hash,
                                               li, totalItems=dlds_len):
                break
